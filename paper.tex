...
\texttt{fib} example from Section~\ref{Sec:OpenMP}. In \figref{tapir-example}
we show the input OpenMP program and a portion of the corresponding Tapir code 
that is generated.  
...
The listed Tapir IR code corresponds to lines 7 through 12 of the C source
...
and found it was similarly outpaced by the Tapir implementation.  \texttt{NQueens}
...
the ways it does. While we have yet to figure out every discrepancy in
performance, we hope to get some ideas for why the performance of our
implementation is generally better and where each of the implementations are
spending a majority of the time. Our primary tool for this task will be performance 
...
issues, etc. Still, they do give us some insight into performance bottlenecks.
...
credit for. We can break reasons for performance discrepancies into a few categories: 
...
For the cases where performance is close between the implementations, like
\texttt{sparselu}, we expect runtimes to be dominated by actual task workloads. This 
expectation is confirmed by investigating performance counters for \texttt{sparselu}
where every implementation spends roughly 90\% of it's time in the task
bodies. 
...
a notion of tasks. \pmnote{but it isn't clear to me that tasks are really being 
used much! :-)}.  The goal of this work is to show that a single intermediate representation can be
used to implement multiple programming paradigms with good performance. Schardl et 
al. showed that Tapir works well as an IR target for Cilk and surmised that it could
...
for one runtime (OpenMP) to use another (the Cilk runtime). While in many
ways this is a weakness of this work, it does open the possibility of utilizing this 
in a more general way to address many aspects including significant challenges 
such as performance portability. Having a parallel-aware IR to work with enables, 
in the same way that LLVM does for serial code, the possibility of multiple high-level 
parallel languages and multiple parallel runtime targets. While the OpenMP specification's 
requirement to have access to runtime routines complicates things, for many programming models
