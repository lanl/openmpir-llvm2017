...
benchmark taken verbatim from the test suite distribution \cite{barcelona}.
...
implementations sometimes write shared variables to memory on every write access. This is a
potential for performance discrepancy because of differing 
implementation behavior.
%Stephen:This topic is not discuessed later, but was touched on before -- moved?
%and we will return to it when discussing
%performance on the Barcelona OpenMP task suite. 
...
\texttt{private} variables are often used for inputs to calculations that are 
not accessed after the tasks complete, 
or for performance rather than their semantic properties.
Also, \texttt{firstprivate} is the default data-sharing attribute for tasks.
