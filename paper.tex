...
Tapir. This enables analyses and optimizations previously inaccessible to
...
analyze and optimize. An ideal solution would be some some form of common
representation of parallelism that all IRs can share.

Internal representations (IRs) are the \wmnote{the phrase compiler objects seems a bit odd to me}compiler objects on which almost all
...
\cite{shardl2017} has shown that real world compiler IRs can be extended to
represent fork-join parallelism. By extending the LLVM instruction set with
three instructions, they were able to capture all of the semantics to implement
the semantics of Cilk, an extension to C. They suggested that a similar
approach could be taken with OpenMP tasks.
...
\wmnote{This feels like we're trying to convince someone about LLVM: perhaps LLVM has gained traction in the academic community for both its simplicity and relatively painless modification}LLVM is arguably the most successful IR to date \cite{llvm}. By implementing a simple
...
Addressing these concerns this is a major motivation for our \wmnote{use of?}turning to Tapir.
\wmnote{this sentence feels very loose/run-on} By standardising a powerful but simple internal representation and building
frontends and backends for it, one both makes it easier to build optimizations
and analyses for a given programming model, and avoids duplicating work
...
worth \wmnote{discussion?}visiting here. We surmise that it is likely due to the variable semantics
