...
This work explores a compiler front end that translates OpenMP tasking semantics to Tapir,
...
a Cilk runtime back end, we compare results to existing OpenMP implementations.
Initial performance results for the Barcelona OpenMP tasking suite show
...
return to this consideration later in Section~\ref{Sec:Discussion} when discussing
future work.
...
a conditional branch in that it is a terminator instruction with with two blocks.
...
replace the two primary \texttt{task\_spawn} and
\texttt{task\_wait} pragmas for task parallelism. By re-using code from Schardl et al.\ for code generation
...
\texttt{task\_wait} constructs, it's worth noting that even for these
pragmas the implementation is incomplete. Currently, any clauses modifying the
behavior of the pragmas is ignored. Probably the most common semantics this
will change is for the semantcis of variables, e.g.\ \texttt{shared} vs.\ \texttt{private}.
Surprisingly, this had little effect on the correctness of the entire Barcelona
OpenMP task suite. Indeed, it is likely that fixing this problem would marginally increase
performance by reducing the number of memory
copies for variables declared \texttt{private} in OpenMP code.  The fact
...
%pragmas in order to run parallel code.
A standard pattern for building
...
the same code in a data parallel manner. For the purpose of this work we took
...
future work. Section~\ref{Sec:Future} discusses how how the 
...
computing the 42nd Fibonacci number. \texttt{FFT} ran with an increase to
\texttt{16M}.  
...
changed program behavior in a couple cases. In the case of \texttt{FFT}, we were
...
In this section we speculate \emph{why} the performance varies in
the ways it does. While we have yet to identify every discrepancy in
performance, we hope to get some ideas for why the performance of our
implementation is generally better and where each of the implementations is
spending the bulk of the execution time. Our primary tool for this task is performance 
counters. It's worth noting that some information is difficult to infer from 
performance counters, such as sources of contention, reasons for memory locality,
etc. Still, they do give us some insight into performance bottlenecks.

Before we begin understanding why, it's worth noting again that the scope of the 
work that this paper is responsible for is only the front end. All performance
gains attributable to the existing Tapir back end and Cilk runtime represent prior work.
...
require deep knowledge of each of the front ends and runtimes. While the
importance of optimizations of parallel code is clear, we hypothesize that most
of the disparity present in these results is more a function of runtime
...
aside), so it is effectively a benchmark of how fast a
...
There are two cases we examing using the profiler. The first is the
...
the runtime. For Tapir the overhead is relatively small given the
size of the work chunks, for \texttt{clang} and \texttt{icc} it is
significantly larger, and for \texttt{gcc} it is overwhelming.
...
large application where some parts were written using Cilk, while other used
OpenMP, and the result would not suffer the incompatibility problems often
seen today.

The surprising fact that program behavior wasn't changed significantly is
worth discussing here. We surmise that it is likely due to the
semantics of variables borrowed from the Cilk codegen and their relation to the OpenMP
specification of semantics. In particular, \emph{OpenMP doesn't specify a
...
potential for performance discrepancy because of differing implementation-defined
behavior and we will return to it when discussing performance on the
...
that in practice this hasn't had an effect because
